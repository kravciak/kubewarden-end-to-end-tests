...
do_upgrade() {
    ...
    if kw_version "=1.36"; then
        local tempdir=$(mktemp -d -p "${BATS_TEST_TMPDIR:-/tmp}" migration-XXX)

        # Fields that are not needed for restore, will be generated by kubernetes
        local FILTER_YQ='del(.items[].metadata.uid, .items[].metadata.resourceVersion, .items[].metadata.creationTimestamp, .items[].metadata.generation, .items[].metadata.managedFields, .items[].status)'
        # Resources managed by kubewarden-defaults chart will be created by helm
        local FILTER_LBL='-l app.kubernetes.io/instance!=kubewarden-defaults -l app.kubernetes.io/name!=kubewarden-defaults'

        kubectl get clusteradmissionpolicies $FILTER_LBL -o yaml | yq "$FILTER_YQ" > "$tempdir/clusteradmissionpolicies-backup.yaml"
        kubectl get admissionpolicies -A -o yaml | yq "$FILTER_YQ" > "$tempdir/admissionpolicies-backup.yaml"
        kubectl get clusteradmissionpolicygroups -o yaml | yq "$FILTER_YQ" > "$tempdir/clusteradmissionpolicygroups-backup.yaml"
        kubectl get admissionpolicygroups -A -o yaml | yq "$FILTER_YQ" > "$tempdir/admissionpolicygroups-backup.yaml"
        kubectl get policyservers $FILTER_LBL -o yaml | yq "$FILTER_YQ" > "$tempdir/policyservers-backup.yaml"

        helm get values kubewarden-crds -n kubewarden -o yaml > "$tempdir/kubewarden-crds-values.yaml"
        helm get values kubewarden-controller -n kubewarden -o yaml > "$tempdir/kubewarden-controller-values.yaml"
        helm get values kubewarden-defaults -n kubewarden -o yaml > "$tempdir/kubewarden-defaults-values.yaml"

        helm uninstall -n kubewarden --wait kubewarden-defaults
        helm uninstall -n kubewarden --wait kubewarden-controller
        helm uninstall -n kubewarden --wait kubewarden-crds

        helm install admission-controller kubewarden/admission-controller -n kubewarden --wait \
            --values "$tempdir/kubewarden-controller-values.yaml" \
            --values "$tempdir/kubewarden-defaults-values.yaml" \
            --values "$tempdir/kubewarden-crds-values.yaml" --devel

        wait_rollout deployment/policy-server-default
        for resource in policyservers clusteradmissionpolicies admissionpolicies clusteradmissionpolicygroups admissionpolicygroups; do
            yq -e '.items | length > 0' "$tempdir/$resource-backup.yaml" &>/dev/null || continue
            kubectl create -f "$tempdir/$resource-backup.yaml"
        done
    else
...
case $1 in
    ...
    up|upgrade)
        if kw_version "=1.36"; then
            load_env
            CONTROLLER_ARGS="${CRDS_ARGS:-} ${DEFAULTS_ARGS:-} ${CONTROLLER_ARGS:-}"
            save_env
        fi;;&
